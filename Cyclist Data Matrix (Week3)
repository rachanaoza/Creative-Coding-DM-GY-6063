/*
** Cyclist Count by Year At Selected Commuter Locations
** data from:
** https://data.cityofnewyork.us/d/m6ar-24vj?category=Transportation&view_name=Cyclist-Count-by-Year-At-Selected-Commuter-Locatio
** your task: create a unique data visualization.
** for more information on working with data in Processing, see: https://processing.org/tutorials/data/
*/

String[] headers;
int[][] data; 

void setup(){
  size(850,1000);
   background(0); // set background color
   smooth();
  // load csv file into String array
  String[] lines = loadStrings("Cyclist_Count_by_Year_At_Selected_Commuter_Locations.csv");
  
  //split the first line into an array and load it into our global headers array
  headers = split(lines[0], ",");
  
  //create a temporary 2D String array.
  //we'll think of it as a 28 row x 9 column matrix
  String[][] tempData = new String[lines.length-1][headers.length];
  
  //Curious to see how many rows and columns we're loading into our tempData?
  //uncomment this line below.
  //println( lines.length-1 + " rows by " + headers.length + " cols.");
  
  //loop through each of the subsequent lines after the header row and
  //assign each to the row index of our tempData matrix 
  for(int i = 1; i < lines.length; i++){
    tempData[i-1] = split(lines[i], ",");//assigns an array to each tempData row
  }
  
  //construct our global data matrix.
  //we can also think of this as a 28 row x 9 col matrix
  data = new int[tempData.length][tempData[0].length];
  
  //loop through each of the rows and columns of our tempData matrix.
  //a.k.a. nested for loops.
  for(int i = 0; i < tempData.length; i++){ //for each row
    for(int j = 0; j < tempData[i].length; j++){ //and for each col
      
      /*
      * since our raw data has "" (quotes) around each entry,
      * we need a way to remove them.  Let's use the handy split() method.
      * The delimiter is a " (double quotes) but we need to escape it using a forward slash.
      * for more info on escape characters in Java see:
      * http://stackoverflow.com/questions/1367322/what-are-all-the-escape-characters-in-java
      * example: split(""1024"", "\"") becomes --> {"", "1024", ""}  
      */
      String[] tempCellValue = split(tempData[i][j], "\"");
      // our middle entry contains the data, so we write tempCellValue[1]
      // handle our blank data OR data containing "N/A" by replacing with 0s 
      if(tempCellValue[1].equals("") || tempCellValue[1].equals("N/A")){
        data[i][j] = 0;
        // println(data[i][j]); // print values equal to 0

      }
      else {
        data[i][j] = Integer.parseInt(tempCellValue[1]);
        // println(data[i][j]);
      }
    }
  }
}


int val=0;   // Tells us the specific row number of the data

    
    
void draw(){
  // draw something awesome with the data matrix and headers array
  // to access individual data cells all you have to do is call data[row][col],
  // where row is an int row number, and col is an int column number
  background(0); // set background as black
  
  beginShape();

 for(int i=1; i<headers.length -1; i++)
 {
   color[] colarray = new color[8];     // this stores the value of colors we would like to represent each header by
    colarray[0]= color(255);
    colarray[1] = color(204 , 0, 102);
    colarray[2] = color(195, 79, 230);
    colarray[3] = color(51, 204, 255);
    colarray[4] = color(102, 204, 51);
    colarray[5] = color(255, 213, 41);
    colarray[6] = color(255, 149, 41);
    colarray[7] = color(223, 120 , 120);
       
    PFont f = loadFont("CenturyGothic-48.vlw"); // load and declare font
   
    textFont(f,15); // declare font size for the names of places
    fill(colarray[i]); // color headers (text) based on the colors mentioned above
    text(headers[i],530,75+(i*20)); // prints the name of the place
    
    if(i<headers.length-2) // to restrict printing of lines to second last point 
    // since we're printing from current point to next point. we don't need to print from last point onwards
    {
       stroke(100); // color of the lines joining each points
       strokeWeight(3); // weight of the lines joining each points
       line(i*80, 625-((data[val][i])/10),((i*80)+80),625-((data[val][i+1])/10)); // plotting lines from one point to another
    }
    
    strokeWeight(17); // size of the points
    stroke(colarray[i]); // colors of each points
    point(i*80,625-((data[val][i])/10)); // location of each points
    
    textFont(f,20); // font size for the year and total
    fill(235);  // font color for the year and total
    text("Year: "+data[val][0],50,100); // print the exact year the data is shown for
    text("Total: "+data[val][8],50,125); // print the exact total of cyclists the data is shown for
    
    stroke(150); // color of x and y axis lines
    strokeWeight(1); // weight of x and y axis lines
    line(20,70,20,720); // the x axis line
    line(20,720,800,720); // the y axis line
 }
  endShape();  
}


void mouseWheel(MouseEvent event) { // this is for the action to take place when you scroll up and down on the trackpad
                                    // if year value reaches the maximum, turn it back to lowest and vice versa
  float e = event.getCount();
  if(e<0)
  {
    if(val==0)
    {
      val=27;
    }
    else
    {
      val=val-1;
    }
  }
  else if(e>0)
  {
    if(val==27)
    {
      val=0;
    }
    else
    {
      val=val+1;
    }
  }
}
